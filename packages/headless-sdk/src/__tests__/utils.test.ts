...
vi.mock('gill', () => ({
  address: vi.fn((addr) => {
    ...
    if (addr === '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM' || 
...
describe('Utils', () => {
  ...
  describe('createPaymentReference', () => {
    ...
    it('should create many unique references', () => {
      const references = Array(100).fill(0).map(() => createPaymentReference())
  ...
  describe('Performance', () => {
    ...
    it('should create payment references quickly', () => {
      const startTime = Date.now()
      const references = Array(1000).fill(0).map(() => createPaymentReference())
      const endTime = Date.now()
