...
export function useFormField({
    ...
    const validateField = useCallback(
        (inputValue: string): string | null => {
            ...
            // Length validations - check if value has content
            if (validation.minLength !== undefined && inputValue.length < validation.minLength) {
                return `Must be at least ${validation.minLength} characters`;
            }

            if (validation.maxLength !== undefined && inputValue.length > validation.maxLength) {
