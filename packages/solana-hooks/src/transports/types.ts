...
export interface Transport {
    ...
    url?: string;
    request<T = unknown>(req: JsonRpcRequest, opts?: TransportRequestOptions): Promise<T>;
}
...
export function computeBackoffDelay(attempt: number, cfg: RetryConfig): number {
    const base = cfg.baseDelayMs ?? 500;
    const max = cfg.maxDelayMs ?? 30_000;
    let delay = cfg.strategy === 'exponential' ? base * Math.pow(2, attempt) : base * (attempt + 1);
