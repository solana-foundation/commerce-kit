...
describe('useTransferToken', () => {
  ...
  describe('Token Transfer Functionality', () => {
    it('should successfully transfer tokens with mock data', async () => {
      const mockTransport = {
        request: vi.fn()
          .mockResolvedValueOnce({ // getAccountInfo (from account exists)
            value: { lamports: 1000000, data: 'mock-data' }
          })
          .mockResolvedValueOnce({ // getAccountInfo (to account doesn't exist)
            value: null
          })
          .mockResolvedValueOnce({ // getLatestBlockhash
            value: { blockhash: 'mock-blockhash', lastValidBlockHeight: 123456 }
          })
          .mockResolvedValueOnce({ // getAccountInfo (to account check for creation)
            value: null
          })
          .mockResolvedValueOnce({ // getSignatureStatuses (confirmation)
    ...
    it('should handle token transfer with existing recipient account', async () => {
      const mockTransport = {
        request: vi.fn()
          ...
          .mockResolvedValueOnce({ // getAccountInfo (to account exists)
            value: { lamports: 1000000, data: 'mock-data' }
          })
          .mockResolvedValueOnce({ // getLatestBlockhash
            value: { blockhash: 'mock-blockhash', lastValidBlockHeight: 123456 }
  ...
  describe('Transfer Functionality', () => {
    ...
    it('should throw error when sender token account does not exist', async () => {
      const mockTransport = {
        request: vi.fn()
          .mockResolvedValueOnce({ // getAccountInfo (from account doesn't exist)
            value: null
          })
      }
    ...
    it('should retry on blockhash expiration', async () => {
      ...
      const mockTransport = {
        request: vi.fn().mockImplementation(() => {
          ...
          if (attempts === 3) {
            return Promise.resolve({ // getAccountInfo (from account exists)
              value: { lamports: 1000000, data: 'mock-data' }
            })
          }
          if (attempts === 4) {
            return Promise.resolve({ // getAccountInfo (to account exists)
              value: { lamports: 1000000, data: 'mock-data' }
            })
          }
          if (attempts === 5) {
            return Promise.resolve({ // getLatestBlockhash
              value: { blockhash: 'fresh-blockhash', lastValidBlockHeight: 123457 }
            })
          }
          return Promise.resolve({ // getSignatureStatuses
            value: [{ confirmationStatus: 'confirmed', err: null }]
          })
        })
      }
  ...
  describe('Form Helpers', () => {
    ...
    it('should throw error when required inputs are missing', async () => {
      ...
      await expect(
        result.current.transferFromInputs()
      ).rejects.toThrow('Mint address, recipient address, and amount are all required')
    })
