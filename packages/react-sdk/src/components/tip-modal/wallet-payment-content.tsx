...
function WalletConnectList({ theme, onIframeConnected }: { theme: Required<ThemeConfig>; onIframeConnected?: (accounts?: Array<{ address?: string }>) => void }) {
  ...
  // Enhanced async handling for wallet connections
  const walletConnection = useAsync(async (walletName: string) => {
    if (typeof (window as any).__IFRAME_WALLET_CONNECT__ === 'function') {
      if (process.env.NODE_ENV !== 'production') {
        console.log('[WalletConnectList][iframe] Requesting parent connect for', walletName);
      }
      const res = await (window as any).__IFRAME_WALLET_CONNECT__(walletName);
      ...
      try { 
        onIframeConnected?.((res as any).accounts); 
      } catch {}
      return res;
    } else if (select) {
    ...
    <div>
      {walletConnection.error && (
        <div className="ck-wallet-error">
          {walletConnection.error.message}
        </div>
      )}
          ...
          {sourceWallets.map((w: any, index: number) => (
            ...
            >
              ...
              {(w as any).installed && (w as any).connectable ? (
                <button
                  onClick={() => walletConnection.execute(w.name)}
                  disabled={walletConnection.loading || connecting}
                  className="ck-wallet-connect-button"
                >
                  {(walletConnection.loading || connecting) && <Spinner size={16} />}
                  {(walletConnection.loading || connecting) ? 'Connecting…' : 'Connect'}
                </button>
              ) : (
                <button
                  onClick={() => {
...
function WalletFlow({ theme, onPaymentComplete, walletIcon, config, selectedAmount, selectedCurrency, customAmount, showCustomInput }: { 
  ...
  // Update payment status based on connection state
  React.useEffect(() => {
    if (connected || iframeConnected) {
      paymentStatus.setStatus('processing');
    }
  }, [connected, iframeConnected, paymentStatus]);
  ...
  }, [connected, onPaymentComplete, paymentStatus]);
  ...
  if (showLoading) {
    ...
    const message = paymentStatus.status === 'success'
      ? 'Payment sent successfully.'
      : paymentStatus.status === 'error'
        ? (paymentStatus.error || 'User declined signature.')
          ...
          <div className="ck-wallet-status-content">
            ...
            {isDev ? (
              <div className="ck-wallet-dev-buttons">
                <button
                  type="button"
                  onClick={async () => { try { await disconnect(); } catch {} paymentStatus.reset(); emittedRef.current = false; setIframeConnected(false); setIframeAccount(null); }}
                ...
                <button
                  type="button"
                  onClick={() => paymentStatus.handleError('User declined signature')}
                  className="ck-wallet-dev-button"
                >
                  Simulate Decline
                </button>
                <button
                  type="button"
                  onClick={() => { paymentStatus.handleSuccess(); onPaymentComplete(); }}
                  className="ck-wallet-dev-button"
                >
                  Simulate Success
                </button>
    ...
    <div className="ck-wallet-flow-container">
      ...
      {process.env.NODE_ENV !== 'production' && (
        <div className="ck-wallet-flow-dev-section">
          <button
            type="button"
            onClick={() => { setIframeConnected(true); paymentStatus.setStatus('processing'); }}
            className="ck-wallet-dev-button"
          >
            Dev: Show Loading
          </button>
          <button
            type="button"
            onClick={() => { setIframeConnected(true); paymentStatus.handleError('User declined signature'); }}
            className="ck-wallet-dev-button"
          >
            Dev: Sad Path
          </button>
          <button
            type="button"
            onClick={() => { setIframeConnected(true); paymentStatus.handleSuccess(); }}
            className="ck-wallet-dev-button"
          >
            Dev: Happy Path
          </button>
          <button
            type="button"
            onClick={() => { setIframeConnected(false); paymentStatus.reset(); setIframeAccount(null); }}
            className="ck-wallet-dev-button"
          >
            Dev: Reset
          </button>
