...
export interface SingleItemCartProps {
  ...
  transactionFeePercent?: number; // e.g., 0.015 for 1.5%
...
export const SingleItemCart = memo<SingleItemCartProps>(({
  products,
  merchant,
  user,
  theme: themeConfig,
  ...
  }, [products, showTransactionFee, transactionFeePercent]);
  ...
  }, [totals.total, selectedCurrency, selectedPaymentMethod, email, products, merchant, onPayment]);
  ...
  return (
    <>
      ...
      <div style={containerStyle} className={`single-item-cart ${className}`}>
            ...
            {products.map((product, index) => (
              <div key={product.id || index} style={{
                ...
                <div style={{
                  ...
                  {formatAmount(product.price, selectedCurrency)}
                </div>
              </div>
            ))}
          </div>
            ...
            <>
              ...
              <button
                ...
                style={{
                  width: '100%',
                  padding: '1rem',
                  backgroundColor: isProcessing || !email.trim() ? '#9ca3af' : theme.primaryColor,
                  color: 'white',
                  border: 'none',
                  borderRadius: getBorderRadius(theme.borderRadius),
                  fontSize: '1rem',
                  fontWeight: '600',
                  cursor: isProcessing || !email.trim() ? 'not-allowed' : 'pointer',
                  transition: 'all 0.2s'
                }}
                type="button"
                onMouseEnter={(e) => {
                  if (!isProcessing && email.trim()) {
              ...
              >
                {isProcessing ? 'Processing...' : `Pay ${formatAmount(totals.total, selectedCurrency)}`}
              </button>
            </>
          ) : (
            <>
...
SingleItemCart.displayName = 'SingleItemCart';