...
describe('useSolanaPay', () => {
  ...
  describe('useSolanaPay hook', () => {
    ...
    it('should initialize with loading state', () => {
      ...
      const { result } = renderHook(() => useSolanaPay(
    ...
    it('should generate unique references for each payment', async () => {
      ...
      const { result: result1 } = renderHook(() => useSolanaPay(
        '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM',
        10,
        'USDC'
      ))

      const { result: result2 } = renderHook(() => useSolanaPay(
  ...
  describe('Error scenarios', () => {
    it('should handle network errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Error creating Solana Pay request:',
        expect.any(Error)
      )
