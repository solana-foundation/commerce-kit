...
describe('Solana Pay Integration', () => {
  ...
  it('should integrate useSolanaPay hook with proper types', () => {
    const { result } = renderHook(() => 
      useSolanaPay(
        'DemoMerchantPublicKey123456789',
        10,
        'USDC'
      )
    )
  ...
  it('should handle different currencies', () => {
    const { result: usdcResult } = renderHook(() => 
      useSolanaPay('merchant123', 5, 'USDC')
    )

    const { result: solResult } = renderHook(() => 
      useSolanaPay('merchant123', 5, 'SOL')
    )
  ...
  it('should handle QR options', () => {
    ...
    const { result } = renderHook(() => 
      useSolanaPay('merchant123', 10, 'USDC', qrOptions)
    )
