...
import { getFeeTypeDecoder, getFeeTypeEncoder, getPolicyDataDecoder, getPolicyDataEncoder, type FeeType, type FeeTypeArgs, type PolicyData, type PolicyDataArgs } from '../types';

export const INITIALIZE_MERCHANT_OPERATOR_CONFIG_DISCRIMINATOR = 2;

export function getInitializeMerchantOperatorConfigDiscriminatorBytes() { return getU8Encoder().encode(INITIALIZE_MERCHANT_OPERATOR_CONFIG_DISCRIMINATOR); }
...
> = Instruction<TProgram>
      & InstructionWithData<ReadonlyUint8Array>
        & InstructionWithAccounts<[TAccountPayer extends string ? WritableSignerAccount<TAccountPayer> & AccountSignerMeta<TAccountPayer> : TAccountPayer, TAccountAuthority extends string ? ReadonlySignerAccount<TAccountAuthority> & AccountSignerMeta<TAccountAuthority> : TAccountAuthority, TAccountMerchant extends string ? ReadonlyAccount<TAccountMerchant> : TAccountMerchant, TAccountOperator extends string ? ReadonlyAccount<TAccountOperator> : TAccountOperator, TAccountConfig extends string ? WritableAccount<TAccountConfig> : TAccountConfig, TAccountSystemProgram extends string ? ReadonlyAccount<TAccountSystemProgram> : TAccountSystemProgram, ...TRemainingAccounts]>
  ;



export type InitializeMerchantOperatorConfigInstructionData = { discriminator: number; version: number; bump: number; operatorFee: bigint; feeType: FeeType; daysToClose: number; policies: Array<PolicyData>; acceptedCurrencies: Array<Address>;  };

export type InitializeMerchantOperatorConfigInstructionDataArgs = { version: number; bump: number; operatorFee: number | bigint; feeType: FeeTypeArgs; daysToClose: number; policies: Array<PolicyDataArgs>; acceptedCurrencies: Array<Address>;  };
...
export function getInitializeMerchantOperatorConfigInstructionDataDecoder(): Decoder<InitializeMerchantOperatorConfigInstructionData> {
  return getStructDecoder([['discriminator', getU8Decoder()], ['version', getU32Decoder()], ['bump', getU8Decoder()], ['operatorFee', getU64Decoder()], ['feeType', getFeeTypeDecoder()], ['daysToClose', getU16Decoder()], ['policies', getArrayDecoder(getPolicyDataDecoder())], ['acceptedCurrencies', getArrayDecoder(getAddressDecoder())]]);
}




export function getInitializeMerchantOperatorConfigInstructionDataCodec(): Codec<InitializeMerchantOperatorConfigInstructionDataArgs, InitializeMerchantOperatorConfigInstructionData> {
  return combineCodec(getInitializeMerchantOperatorConfigInstructionDataEncoder(), getInitializeMerchantOperatorConfigInstructionDataDecoder());
}



export type InitializeMerchantOperatorConfigAsyncInput<TAccountPayer extends string = string,
...
systemProgram?: Address<TAccountSystemProgram>;version: InitializeMerchantOperatorConfigInstructionDataArgs["version"];
...
accounts.config.value = await getProgramDerivedAddress({ programAddress: 'commkU28d52cwo2Ma3Marxz4Qr9REtfJtuUfqnDnbhT' as Address<'commkU28d52cwo2Ma3Marxz4Qr9REtfJtuUfqnDnbhT'>, seeds: [getUtf8Encoder().encode("merchant_operator_config"), getAddressEncoder().encode(expectAddress(accounts.merchant.value)), getAddressEncoder().encode(expectAddress(accounts.operator.value)), getU32Encoder().encode(expectSome(args.version))] });
}
...
// Remaining accounts.
const remainingAccounts: AccountMeta[] = args.acceptedCurrencies.map((address) => ({ address, role: AccountRole.READONLY }))
    ...
    const instruction = {accounts: [
          ...
          data: getInitializeMerchantOperatorConfigInstructionDataEncoder().encode(args as InitializeMerchantOperatorConfigInstructionDataArgs),
      } as InitializeMerchantOperatorConfigInstruction<TProgramAddress, TAccountPayer, TAccountAuthority, TAccountMerchant, TAccountOperator, TAccountConfig, TAccountSystemProgram>;

      return instruction;
  }


export type InitializeMerchantOperatorConfigInput<TAccountPayer extends string = string,
  TAccountAuthority extends string = string,
  TAccountMerchant extends string = string,
  TAccountOperator extends string = string,
  TAccountConfig extends string = string,
  ...
  instruction: Instruction<TProgram>
    ...
    programAddress: instruction.programAddress,
          accounts: {
              ...
              data: getInitializeMerchantOperatorConfigInstructionDataDecoder().decode(instruction.data),
      };
}
