...
describe("PolicyData", () => {
  ...
  it("should serialize settlement policy data correctly", () => {
    const encoder = getPolicyDataEncoder();
    const settlementData: PolicyDataArgs = policyData("Settlement", [
      mockSettlementPolicy,
    ]);
    const serialized = encoder.encode(settlementData);

    expect(serialized).toBeInstanceOf(Uint8Array);
    expect(serialized[0]).toBe(1); // Settlement enum value
  });
  ...
  it("should deserialize policy data correctly", () => {
    ...
    expect(refundDeserialized.fields[0].maxAmount).toBe(
      mockRefundPolicy.maxAmount
    );
    expect(refundDeserialized.fields[0].maxTimeAfterPurchase).toBe(
      mockRefundPolicy.maxTimeAfterPurchase
    );
    ...
    expect(settlementDeserialized.fields[0].minSettlementAmount).toBe(
      mockSettlementPolicy.minSettlementAmount
    );
    expect(settlementDeserialized.fields[0].autoSettle).toBe(
      mockSettlementPolicy.autoSettle
    );
  });
  ...
  it("should handle edge cases for policy data", () => {
    ...
    const minRefundData: PolicyDataArgs = policyData("Refund", [
      minRefundPolicy,
    ]);
    ...
    const manualSettlementData: PolicyDataArgs = policyData("Settlement", [
      manualSettlementPolicy,
    ]);
